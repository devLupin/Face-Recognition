...
with tf.Graph().as_default():
    with tf.Session() as sess:
        ...
        all_id = os.listdir(VALIDATION)
        for id in all_id:
            cur_path = os.path.join(VALIDATION, all_id)
            cur_img_list = os.listdir(cur_path)
            
            for img in cur_img_list:
                # preprocess faces
                cur_img = cv2.imread(os.path.join(cur_path, img))
                
                h, w, _ = cur_img.shape
                
                cur_img = cv2.cvtColor(cur_img, cv2.COLOR_BGR2RGB)
                cur_img = cv2.resize(cur_img, (640, 480))
                cur_img_mean = np.array([127, 127, 127])
                cur_img = (cur_img - cur_img_mean) / 128
                ...
                for i in range(boxes.shape[0]):
                    ...
                    aligned_face = fa.align(cur_img, gray, dlib.rectangle(left = x1, top=y1, right=x2, bottom=y2))
