...
for id in allFolders:
    train_names.append(id)
    
    cur_id = os.path.join(train_path, id)
    temp = []
    
    cur_files = os.listdir(cur_id)
    for img in tqdm(cur_files, desc=id + ' ing....'):
        load = fr.load_image_file(os.path.join(cur_id, img))
        encoding = fr.face_encodings(load)
